---
title: Documentation Standards
id: documentation-standards
section: development
category: guides
created: '2025-03-18'
updated: '2025-03-18'
version: 0.1.0
cursor_rule: true
glob: '**/*'
priority: 500
---

---
title: "Documentation Standards"
id: "documentation-standards"
section: "development"
category: "guides"
created: "2023-10-15"
updated: "2023-12-01"
version: "1.0.0"
contributors: ["developer1"]
tags: ["documentation", "standards", "guidelines"]
related: ["code-style"]
cursor_rule: true
glob: "**/*.md,**/*.c,**/*.h"
priority: 900
rule_category: "code-style"
---

# Documentation Standards

## Core Principles
- Every significant component must be documented
- Documentation lives in `docs/_source` directory
- Code comments reference documentation by ID
- Use standardized format with proper frontmatter

## Document Structure

### Required Frontmatter
```yaml
---
title: "Document Title"
id: "unique-document-identifier"
section: "development|gameplay"
category: "guides|architecture|reference"
created: "YYYY-MM-DD"
updated: "YYYY-MM-DD"
version: "MAJOR.MINOR.PATCH"
---
```

### Optional Frontmatter
```yaml
contributors: ["developer1", "developer2"]
tags: ["tag1", "tag2"]
related: ["related-doc-id1", "related-doc-id2"]
cursor_rule: true  # If this should become a cursor rule
github_doc: true   # If this should be published as a GitHub doc
publish_to: ["wiki", "github"]  # Output targets
```

### Content Guidelines
- Use proper Markdown formatting
- Start with a level-1 heading matching the title
- Use level-2 headings for major sections
- Use level-3 headings for subsections
- Include code examples with proper language indicators
- Keep line length under 120 characters
- Use relative links to other documentation files

## Implementation Pattern

### Documentation References in Code
```c
/**
 * Function description
 * 
 * @see docs:dev-gameplay-movement
 */
void player_movement_handler(player_t *player, movement_type_t move_type)
{
    // Implementation
}
```

### History Tracking
Always maintain a history section at the end of your document:

```markdown
## History
- **2023-12-01**: Updated section X with clarification on Y
- **2023-10-15**: Initial document creation
```

## Naming Convention

Standardized document IDs follow the pattern:
```
{section}-{category}-{descriptor}
```

Examples:
- `dev-ai-multiagent-system`
- `gameplay-guide-character-creation`
- `dev-architecture-memory-management`

## Build Process

Documentation is automatically processed by our build system:
1. Changes in source files trigger documentation rebuild
2. Documentation is compiled to multiple formats
3. Output is generated for wiki, cursor rules, and GitHub

## History
- **2023-12-01**: Added build process information
- **2023-10-15**: Initial documentation standards 